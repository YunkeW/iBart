...
#' @param constant_transformation Logical flag for enabling constant transformations of features. Default is \code{constant_transformation = FALSE}.
#' @param const_transform_threshold Numeric threshold for BART importance scores to consider features for constant transformation. Default is \code{const_transform_threshold = 0.5}.
#' @param const_grid_search Logical flag for using grid search before BFGS optimization. Default is \code{const_grid_search = TRUE}.
#' @param const_grid_values Numeric vector of constants to try in grid search. Default is \code{const_grid_values = c(0.1, 0.5, 1, 2, 5, 10)}.
#' @param const_transform_types Character vector specifying which types of constant transformations to consider. Options are "mult" (multiplicative), "add" (additive), or both. Default is \code{const_transform_types = c("mult")}.
...
iBART <- function(X = NULL, y = NULL,
                  ...
                  const_transform_threshold = 0.5,
                  const_grid_search = TRUE,
                  const_grid_values = c(0.1, 0.5, 1, 2, 5, 10),
                  const_transform_types = c("mult"),
  ...
  for (i in 1:iter) {
    ...
    ### BART-G.SE ###
    bart_result <- BART_iter(data = dat,
                     ...
                     seed = seed,
                     iter = i)
    dat <- bart_result$data
    bart_machine <- bart_result$bart_machine

    # Apply constant transformations if enabled
    if (constant_transformation) {
      # Initialize best_rmse with current model's RMSE
      X_df <- as.data.frame(dat$X)
      colnames(X_df) <- dat$name
      X_df <- X_df[, colnames(bart_machine$training_data_features), drop = FALSE]
      best_rmse <- sqrt(mean((dat$y - predict(bart_machine, new_data = X_df))^2))
      
      # Get BART importance scores
      bart_importance <- bart_machine$var_importance
      
      # Identify features above threshold
      important_features <- which(bart_importance > const_transform_threshold)
      
      if (length(important_features) > 0) {
        # Create list to store transformed features
        transformed_features <- list(
          X = matrix(nrow = nrow(dat$X), ncol = 0),
          unit = NULL,
          name = c()
        )
        ...
        for (feat_idx in important_features) {
          feat_data <- list(
            X = as.matrix(dat$X[, feat_idx, drop = FALSE]),
            unit = if (!is.null(dat$unit)) dat$unit[, feat_idx, drop = FALSE] else NULL,
            name = dat$name[feat_idx]
          )
          
          # Try each transformation type
          for (transform_type in const_transform_types) {
            # First try grid search
            if (const_grid_search) {
              grid_result <- optimize_const_grid(
                feat_data$X, dat$y, 
                const_grid_values, 
                transform_type
              )
              
              # If grid search shows promise, try BFGS
              if (grid_result$rmse < best_rmse) {
                bfgs_result <- optimize_const_bfgs(
                  feat_data$X, dat$y,
                  grid_result$const,
                  transform_type
                )
                ...
                if (transform_type == "mult") {
                  trans_feat <- MULT_CONST(feat_data, bfgs_result$const)
                } else {
                  trans_feat <- ADD_CONST(feat_data, bfgs_result$const)
                }
                
                # Add to transformed features
                transformed_features$X <- cbind(transformed_features$X, trans_feat$X)
                transformed_features$name <- c(transformed_features$name, trans_feat$name)
                if (!is.null(trans_feat$unit)) {
                  transformed_features$unit <- cbind(transformed_features$unit, trans_feat$unit)
                }
              }
            }
          }
        ...
        # Add transformed features to dat if any were created
        if (ncol(transformed_features$X) > 0) {
          dat$X <- cbind(dat$X, transformed_features$X)
          dat$name <- c(dat$name, transformed_features$name)
          if (!is.null(dat$unit)) {
            dat$unit <- cbind(dat$unit, transformed_features$unit)
          }
        }
      }
    }
