...
#' Multiplicative constant transformation
#' @param dat List containing X (feature matrix), unit (units), and name (feature names)
#' @param const Numeric constant to multiply by
#' @return List with transformed features
MULT_CONST <- function(dat, const) {
  if (is.null(dat$unit)) {
    df_out <- list(
      X = dat$X * const,
      unit = NULL,
      name = paste0(const, "*", dat$name)
    )
  } else {
    df_out <- list(
      X = dat$X * const,
...
#' @return List with transformed features
ADD_CONST <- function(dat, const) {
  if (is.null(dat$unit)) {
    df_out <- list(
      X = dat$X + const,
      unit = NULL,
      name = paste0("(", dat$name, "+", const, ")")
    )
  } else {
    df_out <- list(
      X = dat$X + const,
...
#' Optimize constant for a feature using grid search
#' @param X Feature matrix
#' @param y Response vector
#' @param const_values Vector of constants to try
#' @param transform_type Type of transformation ("mult" or "add")
#' @return Best constant value and corresponding RMSE
optimize_const_grid <- function(X, y, const_values, transform_type) {
  best_rmse <- Inf
  best_const <- NA
  ...
  return(list(const = best_const, rmse = best_rmse))
}

#' Optimize constant using BFGS
#' @param X Feature matrix
#' @param y Response vector
#' @param init_const Initial constant value
#' @param transform_type Type of transformation ("mult" or "add")
#' @return Optimized constant value and corresponding RMSE
optimize_const_bfgs <- function(X, y, init_const, transform_type) {
  # Objective function to minimize
  obj_fun <- function(const) {
    if (transform_type == "mult") {
      X_trans <- X * const
    } else {
      X_trans <- X + const
  ...
  # Optimize using BFGS
  result <- optim(init_const, obj_fun, method = "BFGS")
  
  return(list(const = result$par, rmse = sqrt(result$value)))
}
